...
\textbf{Where the music lives.} Engraved examples are not typed into \verb|main.tex|. Each fragment normally lives in its own \verb|.ly| file under the chapter tree (inline, excerpt, or full-page areas). Your chapter \verb|.tex| files import those with \verb|\lilypondfile{./path/to/score.ly}| inside the book's music wrappers (\verb|\beginBookExample| and friends). The \verb|lilypond-book| step turns LilyPond into graphics and LaTeX glue; \verb|make check-ly| can still run plain LilyPond on the same \verb|.ly| files for fast error checking.
