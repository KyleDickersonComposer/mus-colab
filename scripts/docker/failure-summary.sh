		...
		if [[ "$file" == "${LOG_DIR}/lilypond-book.log" ]]; then
			...
			if [[ -n "$failing_ref" ]]; then
				...
				if [[ -n "$owner_tex" ]]; then
					...
					echo "Likely source file: (no ${ly_base}-systems.tex in ${OUTPUT_DIR}; see .tex paths in lilypond-book.log below)"
					echo "Recent source paths in lilypond-book.log:"
					grep -E -o '[A-Za-z0-9_./-]+\.(tex|lytex)' "$file" 2>/dev/null | tail -n 12 | while read -r p; do echo "  $p"; done || true
					note_line="$(
						awk '
							hit && !seen {
								sub(/^[[:space:]]+/, "")
					...
					if [[ -n "$note_line" ]]; then
						source_hits="$(
							grep -R -n -F "$note_line" chapters frontmatter source main.tex preface.tex title-page.tex source-note.tex 2>/dev/null | head -n 5
						)"
